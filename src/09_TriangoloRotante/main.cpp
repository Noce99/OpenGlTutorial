...
void display (GLFWwindow* window, double currentTime){
  ...
  size_var += sizeinc;
  if (size_var>=3.0 || size_var<=0.1){
    sizeinc *= -1;
  }
  GLuint angleLoc = glGetUniformLocation(renderingProgram, "angle");
  glProgramUniform1f(renderingProgram, angleLoc, angle);
  GLuint sizeLoc = glGetUniformLocation(renderingProgram, "size");
  glProgramUniform1f(renderingProgram, sizeLoc, size_var);
